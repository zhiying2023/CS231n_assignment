...
class ThreeLayerConvNet(object):
    ...
    def __init__(
        ...
        #第一个卷积层尺寸不变，pooling层尺寸缩小了一半
        H2=(H-1)/stride+1
        W2=(W-1)/stride+1
        H3=int(H2//2)
        W3=int(W2//2)
        self.params['W2']=np.random.normal(loc=0,scale=weight_scale,size=(num_filters*H3*W3,hidden_dim))
    ...
    def loss(self, X, y=None):
        ...
        loss+=0.5*self.reg*np.sum((W1*W1).sum()+(W2*W2).sum()+(W3*W3).sum())
        cache=caches[-1]
        dx,dW3,db3=affine_backward(dx,cache)
        grads['W3']=dW3+self.reg*W3
        grads['b3']=db3
        cache=caches[-2]
        dx,dW2,db2=affine_relu_backward(dx,cache)
        grads['W2']=dW2+self.reg*W2
        grads['b2']=db2
        cache=caches[-3]
        #池化后尺寸减小一半，将dx与之纬度相匹配
        dx=dx.reshape(X.shape[0],W1.shape[0],int(X.shape[2]//2),-1)
        dx,dW1,db1=conv_relu_pool_backward(dx,cache)
