...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        for i in range(self.num_layers - 1):
            w = np.random.normal(
                loc=0, scale=weight_scale, size=(layer_dims[i], layer_dims[i + 1])
            )
            b = np.zeros(layer_dims[i + 1])
            self.params["w" + str(i + 1)] = w
            self.params["b" + str(i + 1)] = b
            # batch normalization,将gamma，beta初始化为0,1
        ...
        self.params["w" + str(self.num_layers)] = np.random.normal(
            loc=0, scale=weight_scale, size=(layer_dims[-2], layer_dims[-1])
        )
        self.params["b" + str(self.num_layers)] = np.zeros(layer_dims[-1])
        pass
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers - 1):
            ...
            #affine-relu-dropout
                ...
                x, cache = affine_relu_drop_forward(
                    x,
                    w,
                    b,
                    self.dropout_param
        ...
        for i in range(self.num_layers, 0, -1):
            ...
            # L-1层
                ...
                #affine-relu-dropout
                    ...
                    dx, dw, db=affine_relu_drop_backward(dx, cache)
                else:
